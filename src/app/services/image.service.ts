...
export class ImageService {
    ...
    getImages(offset, limit, catId, otherParamas, token, imageIds = []){
        let url = this.host+'webservices/images';
        var dataToSend = {};
        if (otherParamas) {
        ...
        if (imageIds.length) {
            dataToSend['images'] = imageIds.join(',');
        }
        return this.http.post(url, dataToSend).map(res => res.json());
    }
    getSingleImage(imageId){
        return this.http.get(this.host+'webservices/imageDetails/'+imageId).map(res => res.json());
    }
    downloadImage(token, imageId, width, height, type=''){
        let headers = new Headers({ 'Content-Type': 'application/json; charset=utf-8' });
        let options = new RequestOptions({ headers: headers });
        return this.http.post(this.host + 'webservices/downloadImage', JSON.stringify({ token: token, id: imageId, width: width, height: height, type: type}), options).map(res=>res.json());
    }
    ...
    purchaseImageByCredits(imageId, loginToken, credits, hideConfirmBox){
        ...
        return this.http.post(this.host + 'webservices/purchaseImageByCredits', data).map(res => res.json());
    }
    addToSearchHistory(loginToken, imageId){
        ...
        return this.http.post(this.host + 'webservices/manageUserSearchHistory', data).map(res => res.json());
    }
}
